"""
beijing_estimator.py — NAVAR-style neural Granger-causal VAR, per-site.
Interface matches make_torch_jacobian_fn: forward(x) -> (preds, None), x:(B,N,K).
Architecture mirrors the V-Dem primary: per-target MLP over the flattened (N*K)
window, hidden=32, 1 hidden layer, tanh. Frozen after fit.
"""
import numpy as np
import torch
