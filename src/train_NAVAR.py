...
def train_NAVAR(data, maxlags=5, hidden_nodes=256, dropout=0, epochs=200, learning_rate=1e-4,
    ...
    # FIX (Issue 14): use the device variable consistently instead of
    # scattered torch.cuda.is_available() checks. This correctly handles
    # CUDA, CPU, and MPS (Apple Silicon) backends.
    ...
    if val_proportion and val_proportion > 0:
        model.eval()
        with torch.no_grad():
            # X_val/Y_val already on device from the device-consistent move above
            preds_v, _ = model(X_val)
            val_loss = criterion(preds_v, Y_val).item()
    else:
        val_loss = None
