...
class DataLoader(object):
    ...
    def prepare_data(self, data, maxlags, normalize, split_timeseries=False, lstm=False):
        ...
        if not lstm:
            ...
            # FIX (Issue 1): was range(T - maxlags - 1), which left the last
            # valid window as all-zeros (silent data corruption). Correct
            # upper bound is T - maxlags so every window is filled.
            for i in range(T - maxlags):
