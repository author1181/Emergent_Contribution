"""
emergent_contribution.py
========================

Reference implementation of the emergent contribution measure E_j, the methods
...
# ===========================================================================
# Core: emergent contribution from a sequence of Jacobians
# ===========================================================================

@dataclass
class EmergentContribution:
    E: np.ndarray                 # (n_components,) emergent contribution per node
...
def emergent_contribution_from_jacobians(
    ...
    *,
    ...
    Returns EmergentContribution with E (per node), normalized weights, and
    metadata.
    """
    if horizon < 1:
...
def emergent_contribution_observed(
    states: np.ndarray, jac_fn: Callable[[np.ndarray], np.ndarray],
    *, n_components: int, maxlags: int, start_idx: int, horizon: int,
    ...
    return emergent_contribution_from_jacobians(
...
def verify_lti_reduction(
    ...
    n = A.shape[0]
    jseq = [A] * (horizon - 1)
    ec = emergent_contribution_from_jacobians(
