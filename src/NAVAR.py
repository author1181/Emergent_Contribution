...
class NAVAR(nn.Module):
    ...
    def forward(self, x):
        ...
        # FIX (Issue 13): use squeeze(dim=-1) instead of squeeze() to avoid
        # collapsing the batch dimension when batch size == 1. Plain squeeze()
        # removes ALL size-1 dimensions, turning (1, N) into (N,) which then
        # fails to broadcast with self.biases of shape (1, N).
        predictions = torch.sum(contributions, dim=1).squeeze(dim=-1) + self.biases
        contributions = contributions.view([-1, self.num_nodes * self.num_nodes, 1])
